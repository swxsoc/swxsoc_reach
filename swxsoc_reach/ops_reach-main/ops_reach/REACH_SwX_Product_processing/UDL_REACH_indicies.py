...
****************                                                                                                                               
In here are functions that make the REACH indicies and then append to those files. 
...
def append_reach_index(startday =  dt.datetime(2017, 2, 9, 0, 0, 0), endday = dt.datetime(2018, 6, 15, 0, 0, 0), txtoutfile = home + '/Desktop/test_grid',
                       cribdir = '../data/', datadir = home + '/Data/REACH/', region_crib = '../data/regions.txt',pngfileout = home + '/Desktop/test_grid',
                       model = 400, flav = 'w', online = 'no', local = 'yes', sat = 1000, BTstep = 24*60*60./5, csvorcdf = 'csv', dos = 'dB',
                       RI_flavor_file = '/Desktop/RI_File.csv'):
...
def make_reach_index(startday =  dt.datetime(2017, 2, 9, 0, 0, 0), endday = dt.datetime(2018, 6, 15, 0, 0, 0), txtoutfile = home + '/Desktop/test_grid',
    ...
    if csvorcdf == 'cdf':
        dosA, dosB, Lm, MLT, Epoch, lat, lon, satID = lr.cdf_reach(startday = startday, endday = endday, model = model,
        ...
        #dos, lat, lon, Epoch = lr.dosgeo(startday = startday, endday = endday, model = model,
                                             #dos = pltdos, flav = flav, upcrib = online, cribdir = cribdir,
        ...
        dosA, dosB, lat, lon, alt, Epoch = lr.now_reach(model = model, dos = 'dAB', flav =flav, upcrib = online, cribdir = cribdir,
                                                        datadir = datadir, locdata = local, sat = sat)
    ...
    if len(Epoch) >= BTstep:
        ...
        #Here we are finding the indicies over specific periods.. the steps that we want the belt index to be taken at.  
        for i in np.arange(Epoch[0], Epoch[-1], dt.timedelta(seconds = BTstep), dtype = dt.datetime):
            ...
            if flav.upper() == 'X':
                ...
                binrate = dosB[index]
            #tempsatID = satID[index]
            #begin2now = np.where(Epoch < i+dt.timedelta(seconds = BTstep*res))
            #b2nregion = region[begin2now]
            #b2nrateB = rateB[begin2now]
            ...
            #Here we are making the time array for the indicies
            RIEpoch = np.append([RIEpoch], [i+dt.timedelta(seconds = 0.5*BTstep)])

        #Here we now write out the reach indicies for the given flavour.
        ...
        with open (txtoutfile+'.csv', 'w') as fh:
            ...
            writer.writerow(['# '])
            writer.writerow(['# Source: The Aerospace Corporation REACH indicies'])
            writer.writerow(['# This file contains the REACH indices for different magnetospheric regions. '])
            if flav.upper() == 'Z':
                writer.writerow(['# This file contains the REACH indices from the model Z dosemiter.'])
                ...
                writer.writerow(['# There are 6 REACH satellites with a model Z dosemiter'])
            if flav.upper() == 'Y':
                writer.writerow(['# This file contains the REACH indices from the model Y dosemiter.'])
                ...
                writer.writerow(['# There are 12 REACH satellites with a model Y dosemiter'])
            if flav.upper() == 'X':
                writer.writerow(['# This file contains the REACH indices from the model X dosemiter.'])
                ...
                writer.writerow(['# There are 20 REACH satellites with a model X dosemiter'])
            if flav.upper() == 'W':
                writer.writerow(['# This file contains the REACH indices from the model W dosemiter.'])
                ...
                writer.writerow(['# There are 14 REACH satellites with a model W dosemiter'])
            if flav.upper() == 'V':
                writer.writerow(['# This file contains the REACH indices from the model V dosemiter.'])
                ...
                writer.writerow(['# There are 7 REACH satellites with a model V dosemiter'])
            if flav.upper() == 'U':
                writer.writerow(['# This file contains the REACH indices from the model U dosemiter.'])
                ...
                writer.writerow(['# There are 5 REACH satellites with a model U dosemiter'])
            ...
            #writer.writerow(['# column 04 Modified Julian Day labeled as MJD'])
            ...
            for i in range(len(RIEpoch)):
                ...
                #tempdatarow = np.array([RIEpoch[i].strftime('%Y'), RIEpoch[i].strftime('%m'), RIEpoch[i].strftime('%d'), np.str(33282.0 + np.array( RIEpoch[i].toordinal()-dt.datetime(1950,1,1).toordinal()) +  RIEpoch[i].hour/24.0 +  RIEpoch[i].minute/24.0/60.0 + RIEpoch[i].second/24.0/60.0/60.0),np.str(satnum[i]), np.str(RISAA[i]), np.str(RIslot[i]), np.str(RIout[i]), np.str(RIPC[i])])
        ...
        axarr.set_title('REACH Indicies from '+ np.min(RIEpoch).strftime("%Y-%m-%d %H:%M:%S") + ' to ' + np.max(RIEpoch).strftime("%Y-%m-%d %H:%M:%S"))
...
def append_reach_index(startday =  dt.datetime(2017, 2, 9, 0, 0, 0), endday = dt.datetime(2018, 6, 15, 0, 0, 0), txtoutfile = home + '/Desktop/test_grid',
    ...
    if csvorcdf == 'cdf':
        dosA, dosB, Lm, MLT, Epoch, lat, lon, satID = lr.cdf_reach(startday = startday, endday = endday, model = model, 
                                                         ...
                                                         #dos = pltdos, flav = flav, upcrib = online, cribdir = cribdir,
                                                         #datadir = datadir, locdata = local,sat = sat, retsatID = 'yes')
    elif csvorcdf == 'csv':
        dosA, dosB,  lat, lon, csvalt, Epoch, emptyfiles, satID = lr.csv_reach(startday = startday, endday = endday,
                                            locdata = local, retsatID = 'yes',model = model, trackempty = 'yes', sat = sat,
                                             dos = 'dAB', flav = flav, upcrib = online, cribdir = cribdir,datadir = datadir)
    ...
    if len(Epoch) >= BTstep:
        ...
        #print('looking in file ' + RI_flavor_file)
        #reader = csv.DictReader(open(RI_flavor_file), delimiter=',')
        #RI_data = {}
        #for row in reader:
        #    for column, value in row.items():
        #        RI_data.setdefault(column, []).append(value)
        ...
        #print('Epoch looks like', old_Epoch[0], type(old_Epoch[0]))
        ...
        #print('there are dates which occur')
        if len(RIEpoch) == 0: 
            print('')
            print('')
            print('')
            ...
            RIEpoch = np.arange(startday, endday, dt.timedelta(seconds = BTstep), dtype = dt.datetime)
                      #np.array([startday + (dt.timedelta(seconds = i*BTstep)) for i in range(np.int((endday - startday).total_seconds()/BTstep))])
        ...
        preRI = np.where(old_Epoch < np.nanmin(RIEpoch))
        #Here we find all the post - append stuff
        postRI = np.where(old_Epoch > np.nanmax(RIEpoch))
        ...
        print('today is ', dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        testbad = np.where(RIEpoch > dt.datetime.now()) 
        if len(RIEpoch[testbad]) > 0:
            print('these bad dates are being added')
            print(RIEpoch[testbad])
            print('this is in loop ', startday)

        #print(np.min(RIEpoch).strftime("%Y-%m-%d %H:%M:%S"))
        #print(np.max(RIEpoch).strftime("%Y-%m-%d %H:%M:%S"))
        #Here we now write out the reach indicies for the given flavour.
        print('making '+ txtoutfile +'.csv' )
        with open (txtoutfile+'.csv', 'w') as fh:
            writer = csv.writer(fh, delimiter =',')
        ...
        axarr[0].semilogy(RIEpoch, RISAA, color = cdr, label = 'SAA Index')
        ...
        axarr[1].semilogy(RIEpoch, RIslot, color = cdk,  label = 'Slot Index')
        ...
        axarr[2].semilogy(RIEpoch, RIout, color = cda, label = 'Outer Zone Index')
        ...
        axarr[3].semilogy(RIEpoch, RIPC, color = cdd, label = 'PC Index')
        ...
        fig.suptitle(flav.upper() + ' REACH Indicies from '+ RIEpoch[0].strftime("%Y-%m-%d %H:%M:%S") + ' to ' + RIEpoch[-1].strftime("%Y-%m-%d %H:%M:%S"), fontsize = 'xx-large')
        plt.savefig(txtoutfile + '.png')
        plt.close()
        
        #fig, axarr = plt.subplots(nrows=1, ncols=1, figsize = (8*5, 8*4.25))
